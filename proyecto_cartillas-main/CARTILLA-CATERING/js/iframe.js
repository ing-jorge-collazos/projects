...
function initializeDraggablesElements() {
    ...
    if (formType == 3) {
        for (i = 1; i <= numQuestions; i++)
            $("#answer" + i).draggable({ revert: true, zIndex: 10000 });
    } else if (formType == 4) {
        $("#answers").sortable({
            containment: "parent",
        ...
        for (var i = 1; i <= numQuestions; i++) {
            $("#answers" + i).sortable({
                ...
                stop: function(event, ui) {
                    ...
                    $("#question" + array[0]).val(array[1] + " " + array[2] + " " + array[3] + " " +
                        array[4] + " " + array[5])
                }
            });
        }
...
function loadData() {
    ...
    if (0 < numTest) {
        ...
        if (data != null) {
            if (formType == 1 || formType == 5 || formType == 7) {
                for (var i = 1; i <= numQuestions; i++) {
                ...
                for (var i = 1; i <= numQuestions; i++) {
                    var texto = data['question' + i];
                    var ans = 0;
                    if (texto == "1" || texto == "a") ans = 1;
                    else if (texto == "2" || texto == "b") ans = 2;
                    else if (texto == "3" || texto == "c") ans = 3;
                    stateSelectedOptionMultiple(i, ans, i + '' + ans);
                }
            }
